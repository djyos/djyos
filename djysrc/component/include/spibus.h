...
struct tagSPI_CB
{
    struct tagRscNode       SPI_BusNode;                //总线资源节点
    struct tagSPI_Buf       SPI_Buf;                    //缓冲区,用于异步发送
    struct tagSemaphoreLCB *SPI_BusSemp;                //SPI总线保护信号量
    struct tagSemaphoreLCB *SPI_BlockSemp;              //简易缓冲区保护信号量
    struct tagSPI_DataFrame Frame;
    struct tagSPI_Device    *CurrentDev;                //占有当前总线的设备
