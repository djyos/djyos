...
/**
  * @brief     Fills each PINSEL_CFG_Type member with its default value.
  *
  * @param  mode  pointer to a PINSEL_CFG_Type structure which will be initialized.
  * @retval   None
  */

void PINSEL_StructInit ( PINSEL_CFG_Type *mode)
{
   mode->Pinmode   = PINSEL_PINMODE_PULLUP;
   mode->OpenDrain = PINSEL_PINMODE_NORMAL;
   mode->Funcnum   = PINSEL_FUNC_0;
}
