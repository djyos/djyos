...
static
FRESULT dir_find (
    _DIR* dp         /* Pointer to the directory object linked to the file name */
)
{
    FRESULT res;
    BYTE c, *dir;
...
)
{
    ...
    if (*path) {    /* If the pointer is not a null */
        for (tt = *path; (UINT)*tt >= (_USE_LFN ? ' ' : '!') && *tt != ':'; tt++) ; /* Find ':' in the path (检查路径中是否含有':') */
...
)
{
    ...
    if (LD_WORD(&fs->win[BS_55AA]) != 0xAA55)   /* Check boot record signature (always placed at offset 510 even if the sector size is >512) */
        return 2;
    /* FAT12与FAT16版本结构体 */
    if ((LD_DWORD(&fs->win[BS_FilSysType]) & 0xFFFFFF) == 0x544146)     /* Check "FAT"(=464154) string */
        return 0;
    /* FAT32版本结构体 */
    ...
    *rfs = fs;                          /* Return pointer to the file system object */
    ...
    if (fmt == 1 || (!fmt && (LD2PT(vol)))) {   /* Not an FAT boot sector or forced partition number */
        for (i = 0; i < 4; i++) {           /* Get partition offset */
            ...
            br[i] = pt[4] ? LD_DWORD(&pt[8]) : 0;/* LBA地址 */
        }
    ...
    /* Determine the FAT sub type 根据簇大小确定FAT类型 */
...
)
{
    ...
    vol = get_ldnumber(&rp);/* 通过路径名来获取volume号 */
    if (vol < 0) return FR_INVALID_DRIVE;
...
#endif
        cfs->fs_type = 0;               /* Clear old fs object(将已挂载的卸载) */
    }
    ...
    FatFs[vol] = fs;                    /* Register new fs object(挂载新的) */
...
#endif
                    {
                        mem_cpy(&sdj, &dj, sizeof (_DIR));   /* Open the sub-directory */
                        sdj.sclust = dclst;
                        res = dir_sdi(&sdj, 2);
                        if (res == FR_OK) {
...
)
{
    ...
    fs->fs_type = 0;
    pdrv = LD2PD(vol);  /* Physical drive(获取设备号) */
    part = LD2PT(vol);  /* Partition (0:auto detect, 1-4:get from partition table)(获取分区号)*/
    ...
    if (fmt == FS_FAT32) {
        n_fat = ((n_clst * 4) + 8 + SS(fs) - 1) / SS(fs);/* 计算FAT表大小 */
        n_rsv = 32;
        n_dir = 0;
    } else {
        n_fat = (fmt == FS_FAT12) ? (n_clst * 3 + 1) / 2 + 3 : (n_clst * 2) + 4;/* 计算FAT表大小 */
    ...
    if (_MULTI_PARTITION && part) {
        ...
        if (sfd) {  /* No partition table (SFD) */
            ...
            ST_DWORD(tbl + 8, 63);          /* Partition start in LBA (分区起始地址) */
            ST_DWORD(tbl + 12, n_vol);      /* Partition size in LBA (分区大小) */
    ...
    /* Create BPB in the VBR (volume区域) */
