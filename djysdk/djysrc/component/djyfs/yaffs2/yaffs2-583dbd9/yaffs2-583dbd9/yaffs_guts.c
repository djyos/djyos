...
int yaffs_guts_ll_init(struct yaffs_dev *dev)
{
    ...
    if (dev->param.start_block == 0) {
        ...
        dev->internal_end_block = dev->param.end_block + 1;/* todo:如果结束块为NAND的结束快，这边的逻辑就错了 */
        dev->block_offset = 1;
    ...
    if (dev->param.inband_tags)/* tags放置在页内，则业内可用空间要减少16Bytes */
        dev->data_bytes_per_chunk =
            dev->param.total_bytes_per_chunk -
    ...
    if (yaffs_init_nand(dev) != YAFFS_OK) {/* 初始化驱动 */
...
int yaffs_guts_initialise(struct yaffs_dev *dev)
{
    ...
    x = dev->param.chunks_per_block * (dev->internal_end_block + 1);/* 页总量 */
    ...
    if (!init_failed && dev->param.n_caches > 0) {/* 初始化成功，建立cache */
        int i;
        void *buf;
        int cache_bytes =
...
#if 0
    // 去除原因:
    // 1. 申请目录会申请OBJ,导致下面内存释放不了，
    // 2. 下面跳过check point逻辑，不会使用到这几个内部目录
    // 3. 下面会重新建立
    if (!init_failed && !yaffs_create_initial_dir(dev))
        init_failed = 1;
#endif

    // 设置上去除了summary功能，因此不会创建summary相关内存
    ...
    if (!init_failed) {
        ...
        if (dev->param.is_yaffs2) {
        	...
        	if (yaffs2_checkpt_restore(dev)) {
                ...
                if (!init_failed && !yaffs_init_blocks(dev))/* 重新分配块信息等空间 */
                    init_failed = 1;
                ...
                if (!init_failed && !yaffs2_scan_backwards(dev))/* 重新扫描设备 */
                    init_failed = 1;
            }
...
int yaffs_get_n_free_chunks(struct yaffs_dev *dev)
{
    ...
    n_free -= n_dirty_caches;/* todo: 去除缓存在cache中的页??? */
    ...
    n_free -= (blocks_for_checkpt * dev->param.chunks_per_block);/* 去除保留给check pointed的块数 */
