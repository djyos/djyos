 ...
 */
/******************************************************************************
*    从当前路径path中剔除dev名(root),将剩余路径和dev结构体返回
* 参数: path --
*      restOfPath -- 返回,路径比对完后,剩余字符串。
******************************************************************************/
                        ...
                        YCHAR **restOfPath)
{
    ...
    list_for_each(cfg, &yaffsfs_deviceList) {
        ...
        while (yaffsfs_IsPathDivider(*p))/* 跳过"/"字符 */
            p++;
        while (yaffsfs_IsPathDivider(*leftOver))/* 跳过"/"字符 */
            leftOver++;

        while (matching && *p && *leftOver) {

            /* Now match the text part */
            while (matching &&/* 对比设备名和路径节点名 */
        ...
        if (*p)
            matching = 0;/* 如果找到dev的话,这里是不应该被执行到的 */

        if (matching && (thisMatchLength > longestMatch)) {
            /* Matched prefix */
            *restOfPath = (YCHAR *) leftOver;
            retval = dev;
            longestMatch = thisMatchLength;/* 名字一致的长度 */
        }
    }
    return retval;
}
/******************************************************************************
*  检查路径的层级和单个文件/目录长度是否正确
* 返回: "-1" -- 路径格式不正确;
*      "0" -- 路径格式正确。
**/
static int yaffsfs_CheckPath(const YCHAR *path)
{
    ...
    return (*path) ? -1 : 0;/* 如果运行到这里,path不为空,就错了 */
}
...
static struct yaffs_dev *yaffsfs_FindMountPoint(const YCHAR *path)
{
    ...
    if (dev && restOfPath && *restOfPath)/* 路径与设备名不一致 */
        dev = NULL;
    return dev;
}
/******************************************************************************
                         ...
                         int *notDir, int *loop)
{
    ...
    if (startDir) {
        ...
        dir = yaffsfs_FindRoot(path, &restOfPath);/* 从当前路径中查找root */
...
int yaffs_mkdir_reldir(struct yaffs_obj *reldir, const YCHAR *path, mode_t mode)
{
    ...
    if (!parent && notDir)
        ...
        if (dir)
            ...
            yaffsfs_SetError(-EEXIST);  /* name exists,重名或已存在 */
        else
...
*    安装,初始化设备等
* 参数: read_only --
*      skip_checkpt -- ">0"表示跳过check point;"0"表示不跳过。
******************************************************************************/
    ...
    if (dev) {
        if (!dev->is_mounted) {
            ...
            if (skip_checkpt) {
                ...
                result = yaffs_guts_initialise(dev);/* 初始化,不带check point */
...
#else
    return yaffs_mount_common(dev, NULL, 0, 1); // 跳过"check point" */
#endif
}
...
Y_LOFF_T yaffs_freespace_common(struct yaffs_dev *dev, const YCHAR *path)
{
    ...
    if (dev && dev->is_mounted) {
        ...
        retVal *= dev->data_bytes_per_chunk;/* 页数量转为字节数量 */
