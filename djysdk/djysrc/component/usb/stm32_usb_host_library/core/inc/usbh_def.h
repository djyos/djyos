...
typedef struct _USBH_HandleTypeDef
{
  ...
  CMD_StateTypeDef      RequestState; // 两个状态:CMD_SEND-CMD_WAIT
  USBH_CtrlTypeDef      Control;
  USBH_DeviceTypeDef    device;
  USBH_ClassTypeDef*    pClass[USBH_MAX_NUM_SUPPORTED_CLASS];
  USBH_ClassTypeDef*    pActiveClass;
  uint32_t              ClassNumber; // 已注册的CLASS数
  uint32_t              Pipes[15];
  __IO uint32_t         Timer; // SOF中断产生的计时
  uint8_t               id;  
  void*                 pData;                  
  void                 (* pUser )(struct _USBH_HandleTypeDef *pHandle, uint8_t id); // 用户注册的处理程序
