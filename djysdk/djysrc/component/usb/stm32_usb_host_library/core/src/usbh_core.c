...
USBH_StatusTypeDef  USBH_Process(USBH_HandleTypeDef *phost)
{
  ...
  case HOST_IDLE :
    ...
    if (phost->device.is_connected)  
    {
      ...
      // USBH_LL_DriverVBUS (phost, FALSE);
      // USBH_Delay(10000); // TODO
      // USBH_LL_DriverVBUS (phost, TRUE);
      USBH_Delay(200); // TODO
      // Djy_EventDelay(200);
      USBH_LL_ResetPort(phost); // 端口复位
#if (USBH_USE_OS == 1)
      osMessagePut ( phost->os_event, USBH_PORT_EVENT, 0);
#endif
    }
  ...
  case HOST_INPUT:
    {
      ...
      if(phost->pUser != NULL)
      {
        phost->pUser(phost, HOST_USER_SELECT_CONFIGURATION); // 用户注册的处理程序
...
{
  /*Stop Host */ 
  USBH_LL_Stop(phost); // 刷新FIFO，清通道，清中断
