...
u8 MSC_DeviceReady = 0;
static u8 s_ReStart;
static u8 Retry;
static s64 s_StTime, s_EdTime;
static u64 s_TimeElapsed;
/**
  * @}
  */ 
    ...
    if(MSC_Handle->current_lun < MSC_Handle->max_lun)
    {
      ...
      case MSC_READ_INQUIRY:
        ...
        else if(scsi_status == USBH_UNRECOVERED_ERROR)
        {
          ...
          USBH_DbgLog("MSC Device unrecovereable error When \"MSC_READ_INQUIRY\"");
        }
      ...
      case MSC_READ_CAPACITY10:   
        ...
        else if(scsi_status == USBH_UNRECOVERED_ERROR)
        {
          MSC_Handle->unit[MSC_Handle->current_lun].state = MSC_IDLE;
          MSC_Handle->unit[MSC_Handle->current_lun].error = MSC_ERROR;
          USBH_DbgLog("MSC Device unrecovereable error When \"MSC_READ_CAPACITY10\"");
        }
        break;
      ...
      case MSC_REQUEST_SENSE:
        ...
        if( scsi_status == USBH_FAIL)
        {
          if(Retry++ < 10)
          {
        	  USBH_DbgLog("MSC Device NOT ready, retry %d times", Retry);
        ...
        else if(scsi_status == USBH_UNRECOVERED_ERROR)
        {
          MSC_Handle->unit[MSC_Handle->current_lun].state = MSC_IDLE;
          MSC_Handle->unit[MSC_Handle->current_lun].error = MSC_ERROR;
          USBH_DbgLog("MSC Device unrecovereable error When \"MSC_REQUEST_SENSE\"");
        }
        else if(scsi_status == USBH_BUSY)
		{
                                     ...
                                     uint32_t length)
{
  ...
  {
    if(((phost->Timer - timeout) > (10000 * length)) || (phost->device.is_connected == 0)) // TODO
    {
      MSC_Handle->state = MSC_IDLE;
      return USBH_FAIL;
    }
