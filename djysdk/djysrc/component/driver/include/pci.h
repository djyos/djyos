...
#define pcie_debug(fmt, args...)        debug_cond(_DEBUG, fmt, ##args)
/*pcie相关配置宏,0表示不配置，1表示配置*/
#define CONFIG_PHYS_64BIT          0    /*如果CPU存储域的地址是64位的，则配置该宏为1，表示64位*/
#define CONFIG_SYS_PCI_64BIT     0  /*如果PCIE总线域的地址是64位的，则配置该宏为1，表示64位*/
#define CONFIG_PCI_NOSCAN        0 //初始化pcie控制器时不扫描pcie设备
...
extern __inline__ void pci_set_region(struct pci_region *reg,
                                      pci_addr_t bus_start, phys_addr_t phys_start, pci_size_t size,
                                      unsigned long flags)
{
...
extern void pci_cfgfunc_config_device(struct pci_controller* hose,
                                      pci_dev_t dev, struct pci_config_table *);
extern int indirect_read_config_byte(struct pci_controller *hose, pci_dev_t dev,
                                     int offset, u8 * val);
extern int indirect_read_config_word(struct pci_controller *hose, pci_dev_t dev,
                                     int offset, u16 * val);
extern int indirect_read_config_dword(struct pci_controller *hose,
                                      pci_dev_t dev, int offset, u32 * val);
extern int indirect_write_config_byte(struct pci_controller *hose,
                                      pci_dev_t dev, int offset, u8 val);
extern int indirect_write_config_word(struct pci_controller *hose,
                                      pci_dev_t dev, int offset, u16 val);
extern int indirect_write_config_dword(struct pci_controller *hose,
                                       pci_dev_t dev, int offset, u32 val);
...
extern int pci_hose_read_config_byte(struct pci_controller *hose, pci_dev_t dev,
                                     int where, u8 *val);
extern int pci_hose_read_config_word(struct pci_controller *hose, pci_dev_t dev,
                                     int where, u16 *val);
...
extern int pciauto_region_allocate(struct pci_region* res, pci_size_t size,
                                   pci_addr_t *bar);
extern void pciauto_setup_device(struct pci_controller *hose, pci_dev_t dev,
                                 int bars_num, struct pci_region *mem, struct pci_region *prefetch,
...
extern int pci_hose_config_device(struct pci_controller *hose, pci_dev_t dev,
                                  unsigned long io, pci_addr_t mem, unsigned long command);

const char * pci_class_str(u8 class);
int pci_last_busno(void);
unsigned int module_init_pcie(void);
#endif    /* _PCI_H_ */
