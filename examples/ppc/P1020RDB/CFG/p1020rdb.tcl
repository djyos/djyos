########################################################################################
# Initialization file for WB8298 board ,change by lst,from P1020RDB_init_core.tcl
# Clock Configuration:
...
proc init_board {} {
	...
	# by lst,base addr from 0xFF000000 change to 0xFE000000
	...
	# by lst,flash size from 16M change to 32M
	...
	# LAWBAR7 - Local Bus Reserved for CS3
	# bit 8 - 31 = 0xF0000000 - base addr
	mem [CCSR 0xce8] = 0x000f0000
	
	# LAWAR7
	# bit 0 = 1 - enable window
	# change by lst
	# bit 7-11 = 00100 - Local Bus
	# bit 26-31 = 010111 16M - size
	mem [CCSR 0xcf0] = 0x80400017

	# LAWBAR8 - Local Bus Reserved for CS4
	# bit 8 - 31 = 0xF0000000 - base addr
	mem [CCSR 0xd08] = 0x000f0000
	...
	# bit 7-11 = 00100 - Local Bus
	# bit 26-31 = 010111 16M - size
	mem [CCSR 0xd10] = 0x80400017

	# LAWBAR9 - Local Bus Reserved for CS5
	# bit 8 - 31 = 0xF0000000 - base addr
	mem [CCSR 0xd28] = 0x000f0000
	...
	# bit 7-11 = 00100 - Local Bus
	# bit 26-31 = 010111 16M - size
	mem [CCSR 0xd30] = 0x80400017

	# LAWBAR10 - Local Bus Reserved for CS6
	# bit 8 - 31 = 0xF0000000 - base addr
	mem [CCSR 0xd48] = 0x000f0000
	...
	#delay before enable
#	wait 500   by lst 试下不延时可以否

	# DDR_SDRAM_CFG
	mem [CCSR 0x2110] = 0xc70c0008
	
	#wait for DRAM data initialization
#	wait 2000   by lst 试下不延时可以否

	#SPI init
	# SPMODE 
#	mem [CCSR 0x7000] = 0x80000404 		by lst driver来设置
	# SPIM - catch all events
#	mem [CCSR 0x7008] = 0x0000FB00 		by lst driver来设置
	# SPMODE0
#	mem [CCSR 0x7020] = 0x25170008 		by lst driver来设置
	...
	if {$BOOT_DEVICE == $NAND_BOOT_DEVICE} {
		...
		if {$ROM_LOC == $GPCM_8BIT_BOOT_LOC} {
			...
			# CS0 - NOR Flash 
			# change by lst
			# BR0 base address at 0xFE000000, port size 16 bit, GPCM, DECC disabled
			mem [CCSR 0x5000] = 0xFE001001 
		}
		# OR0 32MB flash size
		mem [CCSR 0x5004] = 0xFE000F87
	...
	# CS2 - CPLD change by lst
	# BR3 base address at 0xEFA00000, port size 8 bit, GCPM 
	mem [CCSR 0x5010] = 0xEFA00801
	# OR3 128KB flash size
	mem [CCSR 0x5014] = 0xFFFE0987
...
proc P1020RDB_init_core {} {
	...
	#   0xEFC00000  0xEFC0FFFF  PEX1 I/O     		64k
	#   add by lst
	#   0xEFC10000  0xEFC1FFFF  PEX2 I/O     		64k
	#   0xEFF00000	0xEFF0FFFF  NAND			    64k
	#   CS3~6,没个片选保留16M空间，CS7舍弃不用，因为没有足够的LAWBAR
	#   0xF0000000  0xF0FFFFFF	CS3					16M
	#   0xF1000000  0xF1FFFFFF	CS4					16M
	#   0xF2000000  0xF2FFFFFF	CS5					16M
	#   0xF3000000  0xF3FFFFFF	CS6					16M
	...
	# by lst,修改：写回->写通，16M->32M,分两个条目映射
	reg ${CAM_GROUP}L2MMU_CAM0 = 0x70000FDAFC080000FE000000FE000001
	# by lst，位偏移标志：         |   |   |   |   |   |   |   |
	# by lst，位偏移标志：         0   16  32  48  64  80  96  112
	reg ${CAM_GROUP}L2MMU_CAM1 = 0x70000FDAFC080000FF000000FF000001
	# by lst，位偏移标志：         |   |   |   |   |   |   |   |
	# by lst，位偏移标志：         0   16  32  48  64  80  96  112
	
	# define 1GB	TLB1 entry 1: 0x00000000 - 0x3FFFFFFF; DDR, cacheable, not guarded
	reg ${CAM_GROUP}L2MMU_CAM2 = 0xA0007FC41C0800000000000000000001
	# by lst，位偏移标志：         |   |   |   |   |   |   |   |
	# by lst，位偏移标志：         0   16  32  48  64  80  96  112
...
# by lst, IVPR and IVOR will be set in initcpus.s
